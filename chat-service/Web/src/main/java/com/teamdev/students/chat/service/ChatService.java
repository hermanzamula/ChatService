...
public class ChatService {
    ...
    public Collection<Message> getMessages(String username) {
		...
		final Collection<Message> messages = MESSAGE_STORAGE.getByDateRange(lastDate, new Date());
		USER_LIST.put(user, new Date());
		return  messages;
    }
    ...
    public Collection<PrivateMessage> getPrivates(final String username) {
		final User user = findUser(username.trim());
		final Date lastDate = USER_LIST.get(user);
		final Collection<PrivateMessage> messages = MESSAGE_STORAGE.getPrivateByDateRange(lastDate, new Date(), user);
		USER_LIST.put(user, new Date());
		return  messages;
    }
